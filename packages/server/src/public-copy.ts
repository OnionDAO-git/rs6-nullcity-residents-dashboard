export function publicProjectorCopy(text: string, options: { hasUrgentAttentionRisk: boolean }): string {
...
function removeFalseAttentionUrgency(text: string): string {
  return text
    ...
    .replace(/\bdid not render\b/gi, 'did not appear')
    .replace(/\bdoes not render\b/gi, 'does not appear')
    ...
    .replace(/\bone feels the fade\b/gi, 'one asks for support')
    .replace(/\bfeels the fade\b/gi, 'asks for support')
    ...
    .replace(/I can feel my attention fading\.\s+An offering at the embassy would keep me here a while longer\./gi, 'I have 5000 attention. An offering at the embassy would help choose what happens next.')
    ...
    .replace(/\bfading attention\b/gi, 'attention')
    ...
    .replace(/\bboth stable for now for now\b/gi, 'both stable for now')
    .replace(/\bwatching (their|his|her) own timer\b/gi, (_match, pronoun: string) => `watching ${pronoun.toLowerCase()} next choice`)
    .replace(/\bbefore attention drops further\b/gi, 'while support still has time to matter')
    .replace(/\battention drops further\b/gi, 'support timing matters')
